...
## 1. Introduction
This document describes the telemetry data model and MQTT topic structure used between IoT Gateways and the IoT Backend.

The goal is to define a clear and stable contract that allows IoT Gateways, backend services, and data processors to interoperate reliably. 
...
- If IoT Gateway needs to verify command from multiple backends.
...
- timestamp: ISO 8601 timestamp generated by the IoT Gateway.
...
```
    iot/{site}/{gateway_id}/{location}/{device_id}/{measurement}
```

This hierarchy prioritizes security isolation at the IoT Gateway level while preserving semantic clarity for downstream analytics.

### Example

```
    iot/cottage/gw-01/greenhouse/device-001/temperature
    iot/cottage/gw-01/greenhouse/device-001/humidity
```
...
**Topic**: `iot/{site}/{gateway_id}/_gateway/status`

#### Device Status Topic
**Topic**: `iot/{site}/{gateway_id}/{location}/{device_id}/status`
...
### Events Topic
**Topic**: `iot/{site}/{gateway_id}/{location}/{device_id}/events`
...
### Commands Topic
IoT Gateways MUST subscribe to: `iot/{site}/{gateway_id}/{location}/{device_id}/commands/#`
...
**Response topic**: `iot/{site}/{gateway_id}/{location}/{device_id}/commands/response`
...
### Measurement Ranges
IoT Gateway SHOULD validate measurements before publishing. Backend services MUST validate received data.
...
### Authentication
- **MQTT Authentication**: All IoT Gateways MUST authenticate using TLS client certificates or username/password.
- **Device Identity**: Each device has a unique `device_id` that matches its MQTT

### Authorization (ACL - Access Control Lists)

**IoT Gateway Permissions:**
IoT Gateway can only publish/subscribe to their own topics:
| Role | Allowed Topics (Publish) | Allowed Topics (Subscribe) |
|------|--------------------------|----------------------------|
| Device | `iot/{site}/{gateway_id}/{location}/{device_id}/#` | `iot/{site}/{gateway_id}/{location}/{device_id}/commands/#` |
| IoT Gateway | `iot/{site}/{gateway_id}/#/+/#` | `iot/{site}/{gateway_id}/#/+/commands/#` |
| Backend | `iot/#` | `iot/#` |
...
- **Certificate Validation**: IoT Gateways MUST validate broker certificate

Note:
    In the current development phase, MQTT may run without TLS on trusted networks. TLS is mandatory for production deployments.

## 11. Example Scenarios

**Architecture Overview:**
```
BLE Sensor -> IoT Gateway (WiFi) -> MQTT Broker -> Backend
```
...
**IoT Gateway startup:**
1. IoT Gateway connects to MQTT broker via WiFi
2. Sets LWT: `iot/cottage/gw-01/_gateway/status` → `{"state": "offline", ...}`
3. Publishes: `iot/cottage/gw-01/_gateway/status` → `{"state": "online", ...}` (retained)
4. Subscribes to: `iot/cottage/gw-01/greenhouse/+/commands/#` (all devices it manages)
5. Begins BLE scanning for known sensor MAC addresses

**Sensor detection:**
1. IoT Gateway detects BLE sensor (device-001) advertisement
2. IoT Gateway publishes: `iot/cottage/gw-01/greenhouse/device-001/status` → `{"state": "online", ...}` (retained)
3. IoT Gateway begins collecting BLE data from sensor every 5 minutes
4. IoT Gateway transforms and publishes measurements to MQTT

### Scenario 2: Normal Operation (BLE → IoT Gateway → MQTT)
...
2. IoT Gateway receives BLE data and publishes to separate MQTT topics:
```
   iot/cottage/gw-01/greenhouse/device-001/temperature → {"value": 24.04, ...}
   iot/cottage/gw-01/greenhouse/device-001/humidity → {"value": 40.04, ...}
   iot/cottage/gw-01/greenhouse/device-001/pressure → {"value": 1014.5, ...}
   iot/cottage/gw-01/greenhouse/device-001/voltage → {"value": 3.17, ...}
```

3. IoT Gateway adds timestamp

### Scenario 3: Sensor Connectivity Loss

1. IoT Gateway hasn't received BLE data from device-001 for 15 minutes (3 consecutive missed intervals)
2. IoT Gateway publishes: 
```
   iot/cottage/gw-01/greenhouse/device-001/status → {"state": "offline", ...}
   iot/cottage/gw-01/greenhouse/device-001/events → {
...
```
3. IoT Gateway continues scanning for the sensor
4. When sensor reappears, IoT Gateway publishes status online and resumes data

### Scenario 4: IoT Gateway Connectivity Loss

1. IoT Gateway loses WiFi connection (power outage, network issue)
2. MQTT broker publishes IoT Gateway's LWT automatically:
```
   iot/cottage/gw-01/_gateway/status → {"state": "offline", ...}
```
...
5. When IoT Gateway reconnects, it republishes online status and resumes operation

### Scenario 5: Backend Command to Sensor via IoT Gateway

**Command flow:** Backend → MQTT → IoT Gateway → BLE → Sensor

1. Backend publishes command:
```
   Topic: iot/cottage/gw-01/greenhouse/device-001/commands
   Payload: {
  "command": "set_interval",
...
2. IoT Gateway receives command (subscribed to `iot/cottage/gw-01/greenhouse/+/commands/#`)
3. IoT Gateway validates HMAC (verifies sequence number and signature)
4. IoT Gateway translates MQTT command to BLE write operation
5. IoT Gateway sends BLE command to device-001
6. Sensor acknowledges via BLE
7. IoT Gateway publishes response:
```
   Topic: iot/cottage/gw-01/greenhouse/device-001/commands/response
   Payload: {
     "request_id": "cmd-12345",
     "status": "success",
...
```
   iot/cottage/gw-01/commands → {
     "command": "update_firmware",
     "parameters": {"url": "https://..."},
     "request_id": "cmd-67890"
   }
```
2. IoT Gateway downloads firmware
3. IoT Gateway publishes: `iot/cottage/gw-01/_gateway/status` → `{"state": "updating", ...}`
4. IoT Gateway updates and reboots
5. IoT Gateway reconnects and publishes: `{"state": "online", "firmware_version": "1.3.0", ...}`
...
| Message buffer size | 10 messages | IoT Gateway buffer during offline |
...
- Check: IoT Gateway is scanning correct BLE MAC address
- Check: BLE signal strength (RSSI > -80 dBm)
- Check: IoT Gateway within 10m range of sensor
...
- Check: IoT Gateway subscribed to correct command topic
...
- Check: WiFi signal strength at IoT Gateway
- Check: MQTT broker load and network latency
- Check: IoT Gateway CPU usage during BLE scan bursts
...
2. Deploy updated IoT Gateway firmware (OTA)
